...
class TestMixedModuleStore(unittest.TestCase):
    ...
    def test_delete_item(self, default_ms):
        ...
        if hasattr(private_vert.location, 'version_guid'):
            published_vert_loc = private_vert.location.replace(
                version_guid=None, branch=ModuleStoreEnum.BranchName.published
            )
            self.assertTrue(self.store.has_item(published_vert_loc), "Missing published item")
        else:
            published_vert_loc = private_vert.location
            self.assertTrue(self.store.has_item(
                published_vert_loc,
                revision=ModuleStoreEnum.RevisionOption.published_only), "Missing published item"
            )
